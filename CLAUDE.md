...
2. Выполни через opencode (см. правило ниже)
3. Прочитай изменённые файлы целиком — не только diff
4. Запусти техническую проверку:
   ```bash
   npx tsc --noEmit && npm run lint
   ```
5. Проверь логику вручную: граничные случаи, порядок DOM-элементов, z-index, условные рендеры
6. Если есть проблемы — отправь правку через opencode
7. Финально одобри или доработай сам
